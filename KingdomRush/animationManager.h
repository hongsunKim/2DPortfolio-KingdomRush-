...
public:
	...
	//처음부터 끝까지
	void addDefAnimation(string animationKeyName, char* imageKeyName,
		int fps, BOOL reverse = FALSE, BOOL loop = FALSE);
	//배열에 담아서
	void addAnimation(string animationKeyName, char* imageKeyName,
		int* playArr, int arrLen, int fps, BOOL loop = FALSE);
	//구간 정해서
	void addAnimation(string animationKeyName, char* imageKeyName,
		int start, int end, int fps, BOOL reverse = FALSE, BOOL loop = FALSE);
