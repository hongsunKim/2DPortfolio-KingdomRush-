...
//처음부터 끝까지
void animationManager::addDefAnimation(string animationKeyName, char* imageKeyName,
	int fps, BOOL reverse, BOOL loop)
{
	...
	//이미지와 애니메이션을 초기화 후 맵에 추가
	_mTotalAnimation.insert(make_pair(animationKeyName, ani));
}

//배열에 담아서
void animationManager::addAnimation(string animationKeyName, char* imageKeyName,
	int* playArr, int arrLen, int fps, BOOL loop)
{
	//이미지 및 에니메이션 초기화
	image* img = IMAGEMANAGER->findImage(imageKeyName);
	animation* ani = new animation;
